...
class OrderedQueue:
    ...
    def step(self, elem):
        self.counter_ += 1
        if self.exit_at_success:
            self.write_and_exit(elem)
        else:
            self.write(elem)
...
class Designer:
    ...
    def __init__(self, cfg, target_pdb_path=None, device=None):
        ...
        if target_pdb_path is None:
            ...
            fixed_seq_len = len(getattr(cfg, "init_sequence", ""))
            self._init_no_target(cfg.free_generation_length + fixed_seq_len)
        else:
    ...
    def classifier_loss(self, x_seqs, class_idx):
        """
        Calculate the classifier loss for the given sequences.
        """
    ...
    def calc_total_loss(
        ...
        if class_w:
            class_m_nlls = self.classifier_loss(x, CLASSIFIER_CLASSES.index(class_idx))
...
def main(cfg: DictConfig) -> None:
    ...
    path = f"{hydra_cfg['runtime']['output_dir']}/{cfg.tasks.fixedbb.accept_reject.energy_cfg.class_idx}.p"
    cfg.save_path = path
